...
import { useState, useEffect } from 'react';
import { useForm } from 'react-hook-form';
import { Plus, Pencil, Trash2, X, Package2, User, Calendar, Mail, Phone, MapPin, ChevronRight, ChevronLeft, Search } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
...
export function BookingManagement() {
  ...
  useEffect(() => {
    // Initialize product search states when items change
    const items = watch('items') || [];
    setProductSearchOpen(new Array(items.length).fill(false));
    setProductSearchTerms(new Array(items.length).fill(''));
  }, [watch('items')?.length]);
  ...
  const addItem = () => {
    const currentItems = watch('items') || [];
    setValue('items', [...currentItems, { product_id: '', quantity: 1 }]);
    setProductSearchOpen([...productSearchOpen, false]); // Add this line
    setProductSearchTerms([...productSearchTerms, '']); // Add this line
  };

  const removeItem = (index: number) => {
    const currentItems = watch('items') || [];
    setValue('items', currentItems.filter((_, i) => i !== index));
    setProductSearchOpen(productSearchOpen.filter((_, i) => i !== index));
...
<PopoverContent className="w-[400px] p-0">
  <Command>
    <CommandInput
      ...
      // Ensure productSearchTerms is initialized as an empty array
      value={productSearchTerms?.[index] || ''} // Use optional chaining and || ''
      onValueChange={(value) => {
        const newSearchTerms = [...(productSearchTerms || [])]; // Create a copy if undefined
